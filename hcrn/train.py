...
def train(cfg):
    ...
    for epoch in range(start_epoch, cfg.train.max_epochs):
        ...
        if cfg.val.flag:
            ...
            #show val result
            valid_acc, preds, gts, v_ids, q_ids = validate(cfg, model, val_loader, device, write_preds=True)
            vocab = train_loader.vocab['question_idx_to_token']
            dict = {}
            with open(cfg.dataset.val_question_pt, 'rb') as f:
                obj = pickle.load(f)
            ...
            json.dump(instances, open(f"{output_dir}/epoch_{epoch}.json","w"))
