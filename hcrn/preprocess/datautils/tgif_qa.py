...
def load_video_paths(args):
    ...
    for idx, gif in enumerate(gif_names):
        ...
        if not os.path.exists(gif_abs_path):
            gif_abs_path = os.path.join(args.video_dir, ''.join([gif, '.mkv']))
...
def process_questions_openended(args):
    ...
    if args.mode in ["train"]:
        csv_data = pd.read_csv(args.annotation_file.format("train_val"))
        print("loading csv from "+args.annotation_file.format("train_val"))
    else:
        csv_data = pd.read_csv(args.annotation_file.format("test"))
        print("loading csv from "+args.annotation_file.format("test"))
    ...
    if args.mode in ['train']:
        ...
        split = video_names.index("v_Obj__zWaZqk")
