...
class ClipBertBaseDataset(Dataset):
    ...
    def _load_video(self, video_id, num_clips=None, clip_idx=None,
        ...
        if self.is_train:
            ...
            raw_sampled_frms = pickle.load(open(f"/img/rand/{video_id}_{temp}","rb"))
        else:
            raw_sampled_frms = pickle.load(open(f"/img/uniform/{video_id}","rb"))

        '''
        blind version
        '''
        # if self.is_train:
        #     raw_sampled_frms = np.zeros((8,3,512,512), dtype=float)
        #     raw_sampled_frms = torch.from_numpy(raw_sampled_frms).type(torch.FloatTensor)
        # else:
        #     raw_sampled_frms = np.zeros((16,3,512,512), dtype=float)
        #     raw_sampled_frms = torch.from_numpy(raw_sampled_frms).type(torch.FloatTensor)
